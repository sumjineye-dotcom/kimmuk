  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      **분석 목표:**
      ...
      1. **대본의 이야기 요약 및 구조 분석** (구조 요약에 포함할 내용):
         
         **A. 이야기 요약:**
         - 대본의 전체 스토리를 3-4문장으로 요약
         - 핵심 메시지와 주제
         - 주요 등장인물 또는 중심 소재
         
         **B. 구조 및 스토리텔링 기법 분석:**
         - 이야기 전개 방식 (오프닝 → 본론 → 결론 등 각 섹션의 역할)
         - 사용된 스토리텔링 기법 (문제제기, 해결, 공감, 반전, 극적 전환 등)
         - 후킹 포인트와 클릭유도 요소 (제목 패턴, 숫자 사용 등)
      ...
      **예시:**
      ...
      - 이야기 요약: "50년 전 묻은 타임캡슐을 발견한 손자가 할아버지의 유언을 읽고 인생이 변화하는 감동적인 이야기"
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            structureSummary: { 
              type: Type.STRING, 
              description: "1) 업로드한 대본의 이야기 요약 (3-4문장), 2) 구조 및 스토리텔링 기법 분석 (3-5문단)" 
            },
            topics: {
              type: Type.ARRAY,
              items: {
...
`).join('\n---\n\n');
    ...
    const response = await ai.models.generateContent({
      ...
      **분석 목표:**
      ...
      1. **대본들의 이야기 요약 및 공통 구조 분석** (구조 요약에 포함할 내용):
         
         **A. 각 대본의 이야기 요약:**
         - 각 대본의 핵심 스토리를 간단히 요약
         - 공통 주제와 메시지 패턴
         
         **B. 공통 구조 및 스토리텔링 기법 분석:**
         - 공통된 이야기 전개 방식 (오프닝, 본론, 결론 각 섹션의 역할)
         - 공통 스토리텔링 기법 (문제제기, 해결, 공감, 반전, 극적 전환 등)
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            structureSummary: { 
              type: Type.STRING, 
              description: '1) 각 대본의 이야기 요약, 2) 공통 구조 및 스토리텔링 기법 분석 (3-5문단)' 
            },
            topics: {
              type: Type.ARRAY,
              items: {
